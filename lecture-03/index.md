...
If we have a language $$L$$, define $$L^{*}$$ as the union of $$\{\epsilon\}$$ with the set of strings $$s$$ such that there exist strings $$s_{1},...,s_{k}$$, all in $$L$$, and $$s=s_{1}\circ s_{2}\circ...\circ s_{k}\}$$. Given an automaton that accepts $$L$$, we can create an automaton that accepts $$L^{*}$$ by connecting all terminating states to the original initial state (which we must make a terminating state so as to accept $$\{\epsilon\}$$) with $$\epsilon$$-transitions.
