...
- $$\delta'(s,a)$$, where S is the set of states $$\{s_{1}...s_{k}\}$$, is defined to be $$\delta(s_{1},a)\cup\delta(s_{2},a)\cup...\cup\delta(s_{k},a)$$, or, alternatively, $$\{q\in Q:\exists s\in S:q\in\delta(s,a)\}$$
...
We also allow for $$\epsilon$$-transitions, where $$\epsilon$$ represents the null character. When we encounter one, we just activate to the states returned by the $$\epsilon$$-transition without consuming a chracter.
...
If we have a language $$L$$, define $$L^{*}$$ as the union of $$\{\epsilon\}$$ with the set of strings $$s$$ such that there exist strings $$s_{1},...,s_{k}$$, all in $$L$$, and $$s=s_{1}\circ s_{2}\circ...\circ s_{k}\}$$. Given an automaton that accepts $$L$$, we can create an automaton that accepts $$L^{*}$$ by creating a new start state (which is also a terminal state so as to accept $$\{\epsilon\}$$), adding an $$\epsilon$$-transition from the new start state to the former start state of the original DFA, and connecting the terminating states of the original DFA to the new start state with an $$\epsilon$$-transition.

![](figure-05.jpg)

*Thanks to Anonymous on Piazza who pointed out mistake in the previous version of the last paragraph - connecting the final state directly to the original start state does not work in all cases, e.g. if there are transitions leading to the original start state.*
